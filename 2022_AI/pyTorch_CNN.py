...
# MNIST 데이터를 가져옴 (training Data) 'Tensor' 라는 자료 구조로
training_data = datasets.MNIST("data", train=True, download=True, transform=ToTensor())

# MNIST 데이터를 가져옴 (non-traning(test) Data) 'Tensor' 라는 자료 구조로
...
class network(nn.Module):  # network model define
    def __init__(self):
        ...
        self.conv = nn.Sequential(                # 소괄호 안의 메서드가 차례대로 실행됨
            nn.Conv2d(1, 6, (3, 3), padding=1),   # in_channels = 1, out_channels = 6, kernel_size = 3       output = 28 * 28 * 6
            nn.ReLU(),                            # ReLu 는 음수는 0, 양수는 그대로 표시
            nn.MaxPool2d(2, 2),                   # max 풀링  kernel_size = 2, stride = 2                     output = 14 * 14 * 6 (2*2 라서 기존 28*28이 14*14로 줄어듦)
    ...
    def forward(self, x):           # 구조를 잡은 network 를 실행 시켜주는 forward
        x = self.conv(x)
